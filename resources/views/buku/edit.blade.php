...
<body>
    ...
    <div class="container">
        <div class="card">
            <h2 style="margin-top: 0; color: #6B8E23;">Edit Data Buku</h2>
            <form action="{{ url('/buku/'.$buku->BukuID) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-group">
                        @foreach($kategori as $k)
                            <label>
                                <input type="checkbox" name="kategori[]" value="{{ $k->KategoriID }}" 
                                {{ $buku->kategori->contains('KategoriID', $k->KategoriID) ? 'checked' : '' }}> 
                                {{ $k->NamaKategori }}
                            </label>
                        @endforeach
                    </div>
                ...
                <button type="submit" class="btn-simpan">Update Data Buku</button>
            </form>
        </div>
    </div>
</body>
